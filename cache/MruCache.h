...
class MruCache : public Cache{
	private:
		...
		vector<StaticObject*, boost::pool_allocator<StaticObject*> > _mruQueue;//_lruQueue.begin() is the oldest object
		/*
		size>0
	...
	public:
		...
		*/
		MruCache(double totalCapacity, const char* cacheContents, const char* objectsReference, int shrink);
