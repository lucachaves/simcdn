...
#include <omnetpp.h>
LruCache::LruCache(double totalCapacity, const char* cacheContents, const char* objectsReference, int shrink){
...
void LruCache::_freeSpace(double size){
	...
	while(size > 0){
		...
		mapPos = this->_cachedObjects.find(this->_lruQueue.at(0)->getId());
...
StaticObject* LruCache::getNonExistingObject(StaticObject* object){
	assert(object);
	...
	for(vector<StaticObject*, boost::pool_allocator<StaticObject*> >::iterator tempIterator = this->_lruQueue.begin(); tempIterator != this->_lruQueue.end(); tempIterator++ ) {
...
StaticObject* LruCache::getExistingObject(int id){
	assert(id>=0);
...
StaticObject* LruCache::getObjectForced(int id){
